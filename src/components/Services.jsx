...
const Services = () => {
  ...
  return (
    <div>
      ...
      <button onClick={forNormalDataHandling} className='px-3 py-1 bg-blue-600 text-white rounded-md'>Change normal data</button> <br />

      <h1 className={ isSecondChanged ? 'text-red-600' : ''}>{second}</h1>
      <button onClick={forImpDataHandling} className='px-3 py-1 bg-blue-600 text-white rounded-md'>Change Large data</button>
    </div>
  )
}
